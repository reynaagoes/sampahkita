const fs = require('fs');
fs.mkdirSync('app/(dashboard)/recycler', { recursive: true });
      ...
      <div style={{maxWidth:"860px",margin:"0 auto",padding:"32px 24px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1px",background:"#e5e7eb",borderRadius:"8px",overflow:"hidden",marginBottom:"24px"}}>
          {[
            ...
            { label: "Total Berat", value: totalBought.toFixed(1) + " kg", accent: false },
          ].map((s) => (
          ...
          {loading ? (
            ...
            batches.length === 0 ? (
              <div style={{padding:"48px",textAlign:"center"}}>
                <p style={{fontSize:"13px",fontWeight:"600",color:"#374151",marginBottom:"4px"}}>Belum ada batch tersedia</p>
                <p style={{fontSize:"12px",color:"#9ca3af"}}>Batch dari pengepul akan muncul di sini</p>
              </div>
            ) : (
              <div>
                {batches.map((batch, i) => (
                  <div key={batch.id} style={{padding:"16px 20px",borderBottom: i < batches.length-1 ? "1px solid #f9fafb" : "none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      ...
                      <div style={{display:"flex",gap:"16px",marginBottom:"3px"}}>
                        ...
                        <span style={{fontSize:"12px",color:"#16a34a",fontWeight:"700"}}>= Rp {(batch.totalWeight * batch.pricePerKg).toLocaleString("id-ID")}</span>
                      </div>
