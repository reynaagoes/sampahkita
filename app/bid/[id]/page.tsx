...
export default function BidDetailPage() {
  ...
  useEffect(() => {
    if (status === "authenticated" && params.id) void fetchDetail()
  }, [status, params.id])

  async function fetchDetail() {
    const res = await fetch("/api/bid/" + params.id).then((response) => response.json())
  ...
  async function placeBid() {
    ...
    if (res.ok) {
      alert("Bid berhasil. Harga sekarang: Rp " + data.newPrice.toLocaleString("id-ID"))
      void fetchDetail()
    } else {
      alert(data.error)
    }
    ...
    <div style={{ minHeight: "100vh", background: "#f9fafb" }}>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
          <div>
            {listing.photoUrl ? (
              ...
              <div style={{ width: "100%", aspectRatio: "1", background: "#f3f4f6", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "60px" }}>IMG</div>
            )}
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "20px", marginBottom: "14px" }}>
              ...
              {!isOwner && listing.status === "ACTIVE" && !isFull && (
                <button onClick={placeBid} disabled={bidding}
                  ...
                  {bidding ? "Memproses..." : "Tawar Rp " + nextPrice.toLocaleString("id-ID")}
                </button>
              )}
              ...
              {isOwner && <div style={{ textAlign: "center", padding: "12px", fontSize: "12px", color: "#9ca3af", border: "1px solid #f3f4f6", borderRadius: "6px" }}>Ini adalah listing milikmu</div>}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "20px", marginTop: "16px" }}>
          ...
          {bidHistory.length === 0 ? (
            <p style={{ fontSize: "13px", color: "#9ca3af", textAlign: "center", padding: "20px 0" }}>Belum ada bid. Jadilah penawar pertama.</p>
          ) : (
            <div>
